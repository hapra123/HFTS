...
def activate_trip_for_registered_user(label):
    """
    Function to activate the trip for a registered user.
    Creates a new record in the `trips` table.
    Hardcodes entry_location as 'Manipal', leaves exit_location and end_time as NULL.
    """
    conn = get_db_connection()
    cursor = conn.cursor()

    try:
        # Get the current time for the trip start
        current_time = datetime.now()

        # Retrieve cust_id from registered_users
        cursor.execute("SELECT cust_id FROM registered_users WHERE name = ?", (label,))
        cust_id = cursor.fetchone()

        if cust_id:
            # Insert a new trip with all required fields
            cursor.execute("""
                INSERT INTO trips (cust_id, name, start_time, end_time, is_active, entry_location, exit_location)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (cust_id[0], label, current_time, None, 'Active', 'Manipal', None))

            conn.commit()
            print(f"Trip activated for user {label}. Start time: {current_time}, entry location: Manipal.")
        else:
            print(f"User {label} not found in the registered_users table.")
    
    except Exception as e:
        logger.error(f"Error activating trip for user {label}: {e}")
    finally:
        conn.close()
...
def detect_faces_and_stream(camera, container_client, logger, get_face_embedding, find_matching_user, current_location):
    while True:
        ...
        try:
            ...
            for face in face_list:
                ...
                if current_time - last_upload_time[label] > upload_delay:
                    ...
                    if label != "Unknown":
                        activate_trip_for_registered_user(label)
                    blob_folder = "unknown" if label == "Unknown" else "registered"
                    blob_path = f"Entry/{blob_folder}/{label}_{uuid.uuid4()}.jpg"
                    _, buffer = cv2.imencode('.jpg', face_crop)
                    if label != "Unknown":
                        spoken_users.add(label)
                        try:
                          play_sound(BEEP_AUDIO_PATH2)
                        except Exception as e:
        ...
        for label, data in list(last_seen_faces.items()):
            if current_time - data['timestamp'] < FACE_RETENTION_SECONDS:
                x, y, w, h = data['coords']
